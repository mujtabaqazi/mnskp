...
if(!function_exists('getAllfiscalYearsOptions')){
	function getAllfiscalYearsOptions($isreturn=false,$selected=NULL){		
		...
		for($x=$years;$x>=$preyears;$x--){
			...
			$output .= '<option value="'.$fiscalyear.'" '.$isSelected.' >'.$fiscalyear.'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getAllQuarterOptions')){
	function getAllQuarterOptions($isreturn=false,$selected=NULL){
		...
		for($x=$years;$x>=$preyears;){
			...
			$output .= '<option value="'.$yearquarter.'" '.$isSelected.' >'.$yearquarter.'</option>';
			if($quarter==1){
				$x--;
				$quarter=4;
			}else{
		...
		for($x=$years;$x>=$preyears;$x--){
			foreach ($months as $num => $monthitem) { 
				if(($x==$years) && ($num > (date('m')+1))){}
					...
					$val = $x."-".$month;
					$selected = ($selectedfmonth==$val)?'selected="selected"':''; 
					$output .= '<option value="'.$val.'" '.$selected.' >'.date('M-Y',strtotime($val)).'</option>';								
				}
			}
		}	
		if($isreturn)
...
if(!function_exists('getlevel_options')){
	function getlevel_options($isreturn=false,$selectedlevel=NULL,$noempty=NULL,$multi=NULL){
		...
		$output = '';
		if($noempty===NULL){
			$output .= '<option value=""> -- All Levels -- </option>';
		}
		$output .= '<option value="3" '.(($selectedlevel==="3")?'selected="selected"':'').'>District</option>';
		if($multi){
			$output .= '<option value="4" '.(($selectedlevel==="4")?'selected="selected"':'').'>Multiple Districts</option>';
		}
		$output .= '<option value="2" '.(($selectedlevel==="2")?'selected="selected"':'').'>Province</option>';
		if($isreturn)
			return $output;
		echo $output;
	}
...
if(!function_exists('getSavedFmonth_options')){
	function getSavedFmonth_options($isreturn=false,$selectedfmonth=NULL){
		...
		foreach($fmonthResult as $key => $val){
			...
			$output .= '<option value="'.$val["fmonth"].'" '.$selected.'>'.date("M Y",strtotime($val["fmonth"]."-01")).'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getDistricts_options')){
	function getDistricts_options($isreturn=false,$distcode=NULL,$selected_dist=false){
		$CI = & get_instance();
		$wc = "province = '3'";
		...
		foreach ($result1 as $onedist) { 
			...
			if(($distcode == $onedist["distcode"]) and $selected_dist == true ){
				$output .= '<option value="'.$onedist["distcode"].'" '.$selected.' >'.$onedist["district"].'</option>';
			}else if(($distcode == $onedist["distcode"]) and $selected_dist == false ){
				$output .= '<option value="'.$onedist["distcode"].'" '.$selected.' >'.$onedist["district"].'</option>';
			}else if($selected_dist == false ){
				$output .= '<option value="'.$onedist["distcode"].'" '.$selected.' >'.$onedist["district"].'</option>';
			}
		}
		if($isreturn)
...
if(!function_exists('getHCP_options')){
	function getHCP_options($isreturn=false,$hcptypeid=NULL){
		...
		$output = '<option value="" >-- Select HCP Type--</option>';
		...
		foreach ($result1 as $singleRow) { 
			...
			$output .= '<option value="'.$singleRow["id"].'" '.$selected.' >'.$singleRow["displayname"].'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getChecklists_options')){
	function getChecklists_options($isreturn=false,$ptype=NULL,$chklstId=NULL){
		...
		//jis ka id exist kerta ha designationlist ki checklistid kay sath , or jis ka designation id
		if($ptype)
...
if(!function_exists('getTehsils_options')){
	function getTehsils_options($isreturn=false,$tcode=NULL,$distcode=NULL,$selected_tehsil=false){
		$CI = & get_instance();
		$wc = "procode = '3'";
		if($distcode)
			$wc .= " AND distcode = '$distcode'";
		...
		foreach ($result1 as $oneteh) { 
			...
			if(($tcode == $oneteh["tcode"]) and $selected_tehsil == true ){
				$output .= '<option value="'.$oneteh["tcode"].'" '.$selected.' >'.$oneteh["tehsil"].'</option>';
			}else if(($tcode == $oneteh["tcode"]) and $selected_tehsil == false ){
				$output .= '<option value="'.$oneteh["tcode"].'" '.$selected.' >'.$oneteh["tehsil"].'</option>';
			}else if($selected_tehsil == false ){
				$output .= '<option value="'.$oneteh["tcode"].'" '.$selected.' >'.$oneteh["tehsil"].'</option>';
			}
		}
		if($isreturn)
...
if(!function_exists('getUCs_options')){
	function getUCs_options($isreturn=false,$uncode=NULL,$distcode=NULL,$selected_uc=false){
		$CI = & get_instance();
		$wc = "procode = '3'";
		if($distcode)
			$wc .= " AND distcode = '$distcode'";
		...
		foreach ($result1 as $oneteh) { 
			...
			if(($uncode == $oneteh["uncode"]) and $selected_uc == true ){
				$output .= '<option value="'.$oneteh["uncode"].'" '.$selected.' >'.$oneteh["un_name"].'</option>';
			}else if(($uncode == $oneteh["uncode"]) and $selected_uc == false ){
				$output .= '<option value="'.$oneteh["uncode"].'" '.$selected.' >'.$oneteh["un_name"].'</option>';
			}else if($selected_uc == false ){
				$output .= '<option value="'.$oneteh["uncode"].'" '.$selected.' >'.$oneteh["un_name"].'</option>';
			}
		}
		if($isreturn)
...
if(!function_exists('get_Entity_Name')){
	function get_Entity_Name($code,$type=''){
		...
		$query 		= "Select coalesce((
			...
			when '$type'='LHW' then lhwname('$code') 
			when '$type'='LHS' then lhsname('$code') 
...
if(!function_exists('getListingTableNew')){
    function getListingTableNew($allData,$actionPath="",$addDel = false,$drilldown=false,$editViewPath="",$addview=false){
		$CI = & get_instance();
		$count = 0;
		$extraClass = "";
		if (strpos($actionPath, 'lqas') !== false) {
		...
		if(is_array($allData) && (count($allData) > 0))
		{
			foreach($allData as $key => $value)
			{
				...
				$count++;
				$startpoint++;
				$returnData .= "<tr class=\"$drilldownClass\"><td style='text-align: center;'>$startpoint<input type=\"hidden\" class=\"rowCode\" value=\"$id\" ></td><td style='text-align: center;'>";
				$returnData .= implode("</td><td style='text-align: center;'>",$value);
				$returnData .= "</td>";
				if($actionPath!="")
				{
					if($addview){
						$returnData .= "<td style='text-align: center;'><a href=\"".base_url().$actionPath."_view/$id\"><i class=\"fa fa-search\"></i></a></td>";
					}
					$returnData .= "<td style='text-align: center;'><a href=\"".base_url().$actionPath."_edit/$id\"><i class=\"fa fa-pencil\"></i></a></td>";
					if($addDel)
					{
						$delPath = base_url().$actionPath."_delete/".$id;
						$returnData .= "<td style='text-align: center;'><a href=\"$delPath\" onclick=\"return confirm('Are you sure you want to delete this?');\"><i class=\"fa fa-trash-o\"></i></a></td>";
					}
				}
				if($editViewPath!="")
				{
					...
					if (array_key_exists('view', $editViewPath)) {
						$returnData .= "<td style='text-align: center;'><a href=\"".base_url().$editViewPath["path"]."_view/".$value[$editViewPath["column"]]."\"><i class=\"fa fa-search\"></i></a></td>";
					}
					if (array_key_exists('edit', $editViewPath)) {
						...
						if($pd || $dho){
							...
							if(isset($value[$approvalKey]) && $value[$approvalKey] == "Approved"){
								...
								$returnData .= "<td style='text-align: center;'><a href=\"".base_url().$editViewPath["path"]."/".$value[$editViewPath["column"]]."\"><i class=\"fa fa-pencil\"></i></a></td>";
							}
						}else{
							$returnData .= "<td style='text-align: center;'><a href=\"".base_url().$editViewPath["path"]."/".$value[$editViewPath["column"]]."\"><i class=\"fa fa-pencil\"></i></a></td>";
						}
...
if(!function_exists('get_de_freeze_day')){
	function get_de_freeze_day(){
		...
		if ($result->num_rows() > 0)
		{
			...
			//default freeze date of next month
			return '15';	
		}
	}
}
//get Yes No class for coloring
...
//get Complete Incomplete class for coloring
...
if(!function_exists('getregister_options')){
	function getregister_options($isreturn=false,$selectedregister=NULL){
		$CI = & get_instance();		
		$output = '<option value="" >-- Select Register--</option>';
		...
		foreach($registerResult as $key => $val){
			...
			$output .= '<option value="'.$val["register"].'" '.$selected.'>'.$val["register"].'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getmodule_options')){
	function getmodule_options($isreturn=false,$selectedmodule=NULL){
		...
		foreach($moduleResult as $key => $val){
			...
			$output .= '<option value="'.$val["module"].'" '.$selected.'>'.$val["module"].'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getaction_options')){
	function getaction_options($isreturn=false,$selectedaction=NULL){
		$CI = & get_instance();		
		$output = '<option value="" >Select Action</option>';
		...
		foreach($actionResult as $key => $val){
			...
			$output .= '<option value="'.$val["action"].'" '.$selected.'>'.$val["action"].'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
...
if(!function_exists('getusername_options')){
	function getusername_options($isreturn=false,$selectedusername=NULL){
		...
		foreach($usernameResult as $key => $val){
			...
			$output .= '<option value="'.$val["username"].'" '.$selected.'>'.$val["username"].'</option>';
		}
		if($isreturn)
			return $output;
		echo $output;
