 ...
 * MY_Calendar Class
 *
 * To extend calendar library and add cutom code for generate method
 *
 * @author 		Imran <rajaimranqamer@gmail.com>
			...
			for ($i = 0; $i < 7; $i++)
			{
				if ($day > 0 && $day <= $total_days)
				{
					...
					if (isset($data[$day]))
					{
						...
						//Custom code by raja imran qamer start (rajaimranqamer@gmail.com)
						if(is_array($data[$day])){
							$moon = str_replace(array('{day}'), array($day), $temp);
							foreach($data[$day] as $key => $val){
								$moon = str_replace('{content}['.$key.']', $val, $moon);
							}
							$out .= $moon;
						}else{
