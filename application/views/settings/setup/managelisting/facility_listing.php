			...
			<div class="filters">
				<div class="row">
					<div class="col-xs-2" style="width: 10% ! important; margin-left: 90%;">
						<a href="<?php echo $basePath; ?>setup/facility_add" class="link-add-new"><img src="<?php echo $assetsPath; ?>images/addnew.png">Add New</a>
					</div>
				</div>
			</div>
			<br>
			<?php echo getListingTableNew($tableData, "settings/Setup/facility",false,false,"",false); ?>
			<?php 
			$style1 = "";
			...
			$(document).ready(function(){
				...
				function filterdata(){
					...
					$.ajax({
						...
						url: "<?php echo $basePath; ?>settings/Setup/facility_listing",
						dataType: "json",
						success: function(result){
							$('#tbody').html('');
							if(result != null){
				...
				$(function(){
					$('body').on('click','ul.search_page_pagination>li>a',function(e){
						...
						$.ajax({
							type:'POST',
							data: $('#filter-form').serialize(),
							url:"<?php echo $basePath; ?>settings/Setup/facility_listing/"+$(this).attr("data-ci-pagination-page"),
							dataType: "json",
							success:function(result){
								$('#tbody').html('');
								if(result != null){
