...
class Checklist_model extends CI_Model {
	...
	function get_checklists($limit=null, $start=1, $where=NULL, $like=NULL,$is_ajax=FALSE)
	{
		...
		{	
			if($is_ajax){
				$this->db->join('checklists_targets','checklists_targets.checklist_id = chklst.id');
				$this->db->where(array('checklists_targets.ptype'=>$where['ptype']));
			}else{
				$this->db->where($where);
			}
