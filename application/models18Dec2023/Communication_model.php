...
class Communication_model extends CI_Model {	
	//=========List all records of supervisors for field managers of cchss function starts=========//
	...
	//=========List all records of supervisors for field managers of cchss function ends=========//
	//=========List all records for plans for field managers of cchss function starts=========//
	...
	//=========List all records for plans for field managers of cchss function ends=========//
	//=========List all records for held visits for field managers of cchss function starts=========//
	public function get_held_visits_for_fm($distcode = 0,$fmonth=NULL)
	{
		...
		$this->db->where("dho_approved_stat(vph.distcode,vph.fmonth)","Approved");//get only approved plans 
		$this->db->where("vpv.visitdate <=",date("Y-m-d"));//get only those visits where visit date less than today
		if($fmonth){}else{$fmonth=date("Y-m");}
		$this->db->where("vph.fmonth",$fmonth);
		if(is_array($distcode)){
			$distcodess=array();
	...
	//=============List all records for held visits for field managers of cchss function ends=============//
	//=============Get details of visit of provided id for field managers of cchss function starts=============//
	...
	//=============Get details of visit of provided id for field managers of cchss function ends=============//
	//=============List all districts and count visits for cchss function starts=============//
	...
	//=============List all districts and count visits for cchss function ends=============//
	//=============List all districts and count plans for cchss function starts=============//
	...
	//=============List all districts and count plans for cchss function ends=============//
	//function to count kpis values for kpis system
	/*public function get_all_kpis_count($fmonth=NULL,$distcode=NULL,$visitDate=NULL)
	{
		$ptype 	= 'all';
		$wc = "vph.id > 0";
		...
		if(is_null($distcode)){
			$this->db->select('vph.distcode as code, 
				...
				count(distinct vpv.visitplanid) as supervisors, 
				count(distinct vpv.id) as plannedvisits, 
				get_held_visits_count(vph.fmonth,vph.distcode) as visitsheld,
				count(distinct vpvc.id) as plannedchecklists, 
				get_prgrm_filled_chklst_count(CASE When vph.distcode::Integer > 0 Then vph.distcode Else \'\' END,vph.fmonth,\''.$ptype.'\') as checklistsfilled,
				count(distinct vpv.facode) as hfinplan,
				get_prgrm_hf_visited_count(CASE When vph.distcode::Integer > 0 Then vph.distcode Else \'\' END,vph.fmonth,\''.$ptype.'\') as hfvisited,
				get_prgrm_visits_confirmed_count(CASE When vph.distcode::Integer > 0 Then vph.distcode Else \'\' END,vph.fmonth,\''.$ptype.'\') as visitsconfirmed 
				...
				where '.$wc.' group by vph.distcode,vph.fmonth order by code',FALSE);
		}else{
			if($distcode===0){
	...
	public function get_quarter_wise_visits_count($fyear=NULL,$distcode=NULL)
	{
		...
		if($fyear){
			$years = explode("-",$fyear);
			//work for all quarters of financial year where clause
			...
			$customqmonth3wc = " and ( vph.fmonth > '".$years[1]."-00' and vph.fmonth < '".$years[1]."-04' ) ";
	...
	//function to count lqas filled values for kpis system
	...
	//function to get counts/list of lqas filled checklists for dhis system
		...
		if(is_null($distcode)){
			$this->db->select("
				...
				(
					select distcode,
					count(*) as totalrep,
					COALESCE(sum(CASE WHEN lqas_ap>79 THEN 1 ELSE 0 END),0) as ap,
	...
	//function to get counts/list of lqas filled checklists with different accuracy percentage and period for dhis system
	public function get_dhis_lqas_withap_data($fmonthfrom,$fmonthto,$distcode=NULL)
	{
		...
		if(is_null($distcode)){
			$this->db->select("
				...
				COALESCE(lqascounts.ap90,0) as lqaswithap90,
				...
				(
					select distcode,
					count(*) as totalrep,
					COALESCE(sum(CASE WHEN lqas_ap>90 THEN 1 ELSE 0 END),0) as ap90,
					COALESCE(sum(CASE WHEN lqas_ap BETWEEN 60 and 90 THEN 1 ELSE 0 END),0) as ap60
	...
	//function to get counts/list of lqas filled checklists for lhw system
	...
	public function get_lqas_visits_visitsheld_data($fmonth){
		$this->db->select("vph.distcode as code, 
			COALESCE(districtname(vph.distcode), 'Provincial') as name, 
			count(distinct vpv.visitplanid) as plans, 
			count(distinct vpv.id) as visits,
			get_held_visits_count(vph.fmonth, CASE When vph.distcode::Integer > 0 Then 
			vph.distcode Else '' END, 0, 'defaultptype', 'defaultfmwc') as visitsheld 
			from visit_plan_visit_checklists as vpvc 
			inner join visit_plan_visits as vpv on vpvc.visitplanvisitsid=vpv.id 
			inner join visit_plan_header as vph on vpv.visitplanid=vph.id 
			where vph.id > 0 and vph.fmonth = '$fmonth' 
			group by vph.distcode, vph.fmonth 
			order by code ",FALSE);
			$records = $this->db->get()->result_array();
		return $records;
	}
	
	public function get_data_accuracy_percentage($fmonth){
		$this->db->select("distcode ,districtname(distcode), 
			COALESCE(sum(CASE WHEN lqas_ap = 100 THEN 1 ELSE 0 END),0) as pass,
			COALESCE(sum(CASE WHEN lqas_ap < 100 THEN 1 ELSE 0 END),0) as fail
			from da_lqas where supervisor_desg ='DHIS-Coordinator' and fmonth='$fmonth' group by distcode ",FALSE);
			$records = $this->db->get()->result_array();
		return $records;
	}
} //class ends
