...
class Plans_model extends CI_Model {	
	...
	public function get_plans_list($fmonth=NULL)
	{
		...
		if($ulevel=='2'){
			if($utype == 'ProExecutive'){
				$wc .= " and b.ptype = '$ptype' and b.supervisorid in (select users.id FROM users INNER JOIN designations desg ON desg.id = users.designation 
				WHERE users.level = '2'  and utype in ('Manager','DEO') and 
				ptype='$ptype') ";
	...
	public function get_plan_info($id)
	{
		...
		// previously it was ulevel 2 so I changed it to 3 so that we can show monthly visit plan data by kpadmin credentials
		if($ulevel=='3' )
		{
			$wc["procode"] = $this -> session -> procode;
		...
		//print_r($records);exit;
		//get visit plan visits of given visit plan id
	...
	public function get_plans_for_pd_approval($fmonth=NULL)
	{
		...
		$this->db->from("users");
		$this->db->join("designations desg","desg.id = users.designation","INNER");
		$this->db->where($uwc);
		$wherein = array("Manager","DEO");
		$this->db->where_in("utype",$wherein);
		$query = $this->db->get()->result_array();//echo $this->db->last_query();exit;
		$res = array();
	...
	public function get_plans_for_dg_approval($fmonth=NULL)
	{
		...
		from (select case when b.distcode<>\'\' then b.distcode else b.ptype end as distcode, case when b.distcode<>\'\' then 
		districtname(b.distcode) else getprogram(b.ptype) end as districtname, a.id, a.facode, a.visitplanid, 
		a.visitdate,b.supervisorid,a.vehicleassigned,a.driver 
		...
		where b.fmonth=\''.$fmonth.'\' '.$ptype.' 
		AND "users"."utype" IN(\'ProExecutive\', \'Executive\') AND "users"."level" IN(\'2\', \'3\')
	...
	public function gettarget_lhs($facode=NULL)
	{
		...
		$this->db->from("lhsdb");
		$this->db->where($wc);
		$records = $this->db->get()->result_array();//echo $this->db->last_query();exit;
		return $records;
	}
