...
class Communication extends CI_Controller {
	...
	public function get_mns_supervisors_list()
	{
		...
		$servercode = md5('cchss'.date("Y-m-d").'to'.$code.'m&s');
	...
	/* public function get_mns_kpis_count()
	{
		$clientcode = $this -> input -> post('hackerinfo');
		$code = $this -> input -> post('code');
		$servercode = md5('m&e'.date("Y-m-d").'to'.$code.'m&s');
		if($servercode == $clientcode)
		{
			$fmonth = $this -> input -> post('fmonth');
			$distcode = $this -> input -> post('distcode');
			$distcode = (isset($distcode) && $distcode>0)?$distcode:NULL;
			//check if fmonth provided
			if($fmonth>0){
				$data["tableData"] = $this -> Communication_model -> get_all_kpis_count($fmonth,$distcode);
	...
	public function get_visits_count()
	{
		$clientcode = $this -> input -> post('hackerinfo');
		$code = $this -> input -> post('code');
		$servercode = md5('dap'.date("Y-m-d").'to'.$code.'m&s');
		if($servercode == $clientcode)
		{
			$fyear = $this -> input -> post('fyear');
			$distcode = $this -> input -> post('distcode');
	...
	public function get_sup_freq_visits_list()
	{
		...
		if($servercode == $clientcode)
		{
			$fquarter = $this -> input -> post('fquarter');//2016-03,2017-01 etc
			$distcode = $this -> input -> post('distcode');
			$distcode = (isset($distcode) && $distcode>0)?$distcode:NULL;
	...
	public function get_dhis_lqas_held_data()
	{
		$clientcode = $this -> input -> post('hackerinfo');
		$code = $this -> input -> post('code');
		$servercode = md5('dhis'.date("Y-m-d").'to'.$code.'m&s');
		if($servercode == $clientcode)
		{
			$fmonth = $this -> input -> post('fmonth');
	...
	public function get_lhw_lqas_held_data()
	{
		$clientcode = $this -> input -> post('hackerinfo');
		$code = $this -> input -> post('code');
		$servercode = md5('lhw-mis'.date("Y-m-d").'to'.$code.'m&s');
		if($servercode == $clientcode)
		{
			$fmonth = $this -> input -> post('fmonth');
			$distcode = $this -> input -> post('distcode');
	...
	public function get_supervisory_visits(){
		$clientcode = $this -> input -> post('hackerinfo');
		$servercode = md5('mnch'.date("Y-m-d").'to m&s');
		//print_r($servercode);exit;
		//$servercode = 123;
		$data["response"] = false;
		if($servercode == $clientcode)
		{
			$fmonth = $this -> input -> post('fmonth');
			if($fmonth){
				$data["data"] = $this -> Communication_model -> get_lqas_visits_visitsheld_data($fmonth);
				set_activity_log($this->module, "M&S LQAS Plan and visits held fetched.", "M&S LQAS Plan and visits held for fmonth $fmonth for all districts . from MNS system");
				$data["response"] = true;
			}else{
				$data["error"] = "Year-Month not/wrong provided";
			}
	...
	public function get_data_accuracy_percentage(){
		$clientcode = $this -> input -> post('hackerinfo');
		$servercode = md5('mnch'.date("Y-m-d").'to m&s');
		$data["response"] = false;
		if($servercode == $clientcode)
		{
			$format_fmonth = false;
			$fmonth = $this -> input -> post('fmonth');
			if (preg_match("/^[0-9]{4}(-|\/)([1-9]|0[1-9]|1[0-2])$/",$fmonth)) {   ///fmonth format = ('Y-m')
				$format_fmonth =  true;
			}
			if($format_fmonth){
				$data["data"] = $this -> Communication_model -> get_data_accuracy_percentage($fmonth);
				set_activity_log($this->module, "M&S LQAS Percentage fetched.", "M&S LQAS Percentage fetched for fmonth $fmonth . from MNS system");
				$data["response"] = true;
			}else{
				$data["error"] = "Year-Month not/wrong provided";
			}
