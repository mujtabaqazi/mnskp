...
<html>
	<head>
		...
		<title>M&S || Designations List</title>
		...
		<div class="container container-listing">
			<div class="row">
				<div class="col-md-12 text-center">
					<div class="bgrowlis">
						<p>Designation Management (List of current Designations)</p> 
					</div>
				</div>
			</div>		
			<div class="filters">
				...
				<div class="row">
					<div class="col-xs-2" style="width: 10% ! important; margin-left: 90%;">
						<a href="<?php echo $basePath; ?>settings/designation_add" class="link-add-new"><img src="<?php echo $assetsPath; ?>images/addnew.png">Add New</a>
					</div>
				</div>
				<?php echo form_close(); ?>
			</div><!--end of div with class filters-->
			<br>
			<?php echo getListingTable($tableData,"settings/designation"); ?>
			...
			$(document).ready(function() {
				function filterdata(){
					...
					$.ajax({
						...
						url: "<?php echo base_url(); ?>settings/designations/0",
						dataType: "json",
				...
				$(function(){
					$('body').on('click','ul.search_page_pagination>li>a',function(e){
						...
						$.ajax({
							type:'POST',
							data: $('#filter-form').serialize(),
							url:"<?php echo base_url(); ?>settings/designations/"+$(this).attr("data-ci-pagination-page"),
							dataType: "json",
							success:function(result){
								$('#tbody').html('');
								if(result != null){
				...
				$(document).on("change","#level",function(){
					...
					if(itsvalue === '3'){
						//district level
						$('#ptype option:nth-last-child(2)').text("DHO Office");
						$('#ptype option:nth-last-child(1)').text("FP IHP Program");
					}
					filterdata();
				});
				//trigger level
