	...
	<body>
   				...
   				<div class="panel-heading text-center"> Monthly Visit Plan</div>
   					<div class="panel-body pbody">
					    ...
					    <div class="row">
          					...
          					<div class="col-xs-3">
            					<input value="<?php $var ="plandate"; echo isset($DataRow)?date("d-m-Y",strtotime($DataRow[$var])):''; ?>" name="<?php echo $var; ?>" class="form-control" readonly="readonly" required="required" type="text">
          					</div>
                            <div class="col-xs-3">
                           		<label class="pt7 pb2">Name of Supervisor</label>
                            </div>
                            <div class="col-xs-3">
                            	<input value="<?php $var ="supervisor"; echo isset($DataRow)?$DataRow[$var]:''; ?>" name="<?php echo $var; ?>" class="form-control" required="required" type="text">
                            </div>
        				</div>
        				<div class="row">
          					<div class="col-xs-3">
                            ...
                            <div class="col-xs-3">
          					...
          					<div class="col-xs-3">
            					<input value="<?php $var ="postingplace"; echo isset($DataRow[$var])?$DataRow[$var]:'Khyber PakhtunKhwa'; ?>" name="<?php echo $var; ?>" class="form-control" readonly="readonly" required="required" type="text">
          					</div>
        				</div>
        				<div class="row">
          					<div class="col-xs-3">
                            ...
                            <div class="col-xs-3">
                                <input value="<?php $var ="fieldvisitsplanned"; echo isset($DataRow)?$DataRow[$var]:''; ?>" name="<?php echo $var; ?>" id="<?php echo $var; ?>" class="form-control" required="required" type="text" readonly>
                            </div>
        				</div>
        				<div class="row">
          					<div class="col-xs-3">
          					...
          					<div class="col-xs-3">
								<p><?php echo isset($DataRow)?date("M Y",strtotime($DataRow["fmonth"])):''; ?></p>
            				</div>
							<div class="col-xs-3">
            					<label class="pt7 pb2"><?php if($ulevel=="3")echo "District";if($ulevel=="2")echo "Province"; ?></label>
          					</div>
          					<div class="col-xs-3">
            					<input class="form-control" type="text" readonly="readonly" value="<?php if($ulevel=="3")echo get_District_Name((isset($DataRow)?$DataRow["distcode"]:$this -> session -> distcode));if($ulevel=="2")echo "Khyber PakhtunKhwa"; ?>" >
          					</div>
        				</div>
						...
						<div class="row bc">
							...
							<?php } ?>
							<div class="col-xs-1 br zp text-center">
								<label>Facility Type</label>
							</div>
							<div class="col-xs-2 br text-left">
								...
								<div class="row">
									<?php if($this -> session -> userLevel == '2'){ ?>
										<div class="col-xs-1">
											<div class="row">
											  ...
											  <div class="col-xs-10 zp">
											  	<select class="form-control" name="visitcategory[<?php echo ($key+1); ?>]" id="visitcategory">
													<option <?php echo ($val["visitcategory"]=="")?'selected="selected"':''; ?> value="">-- Select Visit Category--</option>
													<!--<option <?php echo ($val["visitcategory"]=="DAP")?'selected="selected"':''; ?> value="DAP">DAP</option>-->
													<option <?php echo ($val["visitcategory"]=="Vertical Program")?'selected="selected"':''; ?> value="Vertical Program">Program</option>
												</select>
											  </div>
											</div>
										</div>
										<div class="col-xs-1 zp">
											<select class="form-control" name="" id="distcode">
											  <?php echo getSelectedDistricts_options(false,$val['facode']); ?>
											 </select>
										</div>
									<?php }else{ ?>
										<div class="col-xs-1">
									...
									<div class="col-xs-1 zp">
										<select class="form-control fatype" required="required">
											<option <?php echo ($val["fatype"]=="")?'selected="selected"':''; ?> value="">Select Facility Type</option>
											<option <?php echo ($val["fatype"]=="DHQ")?'selected="selected"':''; ?> value="DHQ">DHQ - District Head Quarter</option>
											<option <?php echo ($val["fatype"]=="THQ")?'selected="selected"':''; ?> value="THQ">THQ - Tehsil Head Quarter</option>
											<option <?php echo ($val["fatype"]=="HOSP")?'selected="selected"':''; ?> value="HOSP">HOSP - HOSPITAL </option>
											<option <?php echo ($val["fatype"]=="RHC")?'selected="selected"':''; ?> value="RHC">RHC - RURAL HEALTH CENTER</option>
											<option <?php echo ($val["fatype"]=="BHU")?'selected="selected"':''; ?> value="BHU">BHU - BASIC HEALTH UNIT</option>
											<option <?php echo ($val["fatype"]=="MCH")?'selected="selected"':''; ?> value="MCH">MCH - MCH CENTRE (MCH)</option>
											<option <?php echo ($val["fatype"]=="ADMIN")?'selected="selected"':''; ?> value="ADMIN">DHO - District Health Office</option>
											<?php								
											$allTypes = array('MD', 'UD', 'RD', 'DISP', 'HOM', 'TD', 'UDIS', 'CD', 'GRD', 'HD');
											?>
											<option <?php echo (in_array($val["fatype"], $allTypes))?'selected="selected"':''; ?> value="Dispensaries">Dispensaries</option>
											<option <?php echo ($val["fatype"]=="school")?'selected="selected"':''; ?> value="school">CMW Schools</option>											
											<option <?php echo ($val["fatype"]=="Others")?'selected="selected"':''; ?> value="Others">OTHER</option>	
										</select>
									</div>
									<div class="col-xs-2 zp">
										<select class="form-control facilities" required="required" name="facode[<?php echo ($key+1); ?>]" >
											<?php echo getFacilities_options(false,$val["facode"],substr($val["facode"],0,3),NULL,NULL,NULL,$val["fatype"]); ?>
										</select>
									</div>
									<div class="col-xs-6">
									...
									<div class="form-group" style="margin-bottom:0px !important;">
										<div class="col-xs-6 zp">
											<select class="form-control checklists" required="required" name="checklistid[<?php echo ($key+1); ?>][<?php echo ($key1); ?>]" >
												<?php echo $checklists_options = getChecklists_options(true,NULL,$vall["checklistid"]); ?>
											</select>            
										</div>
										<div class="col-xs-2 zp">
											<select class="form-control targets" required="required" name="hcptype_id[<?php echo ($key+1); ?>][<?php echo ($key1); ?>]">
												<?php echo $hcptypeOptions = hcpTypeName(true,$vall["hcptype_id"]); ?>
											</select>
										</div>
										<div class="<?php if($key1 == 0){ echo "col-xs-4"; }else{ echo "col-xs-3"; } ?> zp hcp">
											<?php echo hcpTypeValue(true,$vall["hcptype_id"],$vall["hcp_value"],$key+1,false,$key1); ?>
										</div>
										<?php if($key1 == 0){}else{ ?>
									...
									<div class="col-xs-1 zp">
										<div class="row">
											<div class="col-xs-12">
												<input style="width:70%;float:left;" value="<?php $var ="visitdate[".($key+1)."]"; echo isset($val)?date("d-m-Y",strtotime($val["visitdate"])):''; ?>" name="<?php echo $var; ?>" class="dp form-control" required="required" type="text" >
												<button style="float:right;margin-top:4%;margin-right:3%;" type="button" class="btn btn-danger btn-xs <?php if($key!=0){ echo "delRow_btn";} ?>" data-original-title="delete record"  data-toggle="tooltip" title="delete record" id="hide"><i  class="fa fa-minus"></i></button>
											</div>
										</div>
						...
						<div class="row" style="background: #0B7D05 none repeat scroll 0% 0%;">
							<div style="text-align: right;padding-top: 5px;padding-bottom: 5px;" class="col-md-5 col-md-offset-7 col-sm-6 col-sm-offset-6 col-xs-6 col-xs-offset-6">
								<button type="submit" id="saveplanbtn" class="saveplanbtn btn btn-primary btn-md btncc" role="button">
									<i class="fa fa-floppy-o "></i> Update Plan  
								</button>
		...
		<script>
			function getStartDate() {
				var yearMonth = '<?php echo $DataRow["fmonth"]; ?>';
