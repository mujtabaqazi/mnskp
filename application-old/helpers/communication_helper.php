	...
	function gettarget_cmws($facode)
	{
		...
		if (($userId > 0) && ($CI -> session -> userLevel > 0)){
			if($facode > 0){
				...
				$validationcode = md5('mns'.date("Ymd").'to'.$random.'mnch');
	...
	function gettarget_lhws($facode)
	{
		...
		if (($userId > 0) && ($CI -> session -> userLevel > 0)){
			if($facode > 0){
				//validation mechanism
				$random = rand(0,99999999).date("Y").date("m");
				$validationcode = md5('mns'.date("Ymd").'to'.$random.'lhwmis');
				$data = array('hackerinfo' => $validationcode, 'code' => $random, 'facode' => $facode);
				//post data mechanism
				$fields_string = "";
				foreach($data as $key=>$value) { $fields_string .= $key.'='.$value.'&'; }
...
if(!function_exists('get_lqas_de_mis_values')){
	function get_lqas_de_mis_values($fmonth,$prgrmtype,$code,$fields)
	{
		...
		if (($userId > 0) && ($CI -> session -> userLevel > 0)){
			...
			switch($prgrmtype){
				case "all":
					$url = DHIS_BASE_URL."get_custom_fields.php";
					$name = "facode";$mis = "dhis";
					break;
				case "lhw":
					$url = LHW_BASE_URL."get_custom_fields";
					$name = "lhwcode";$mis = "lhwmis";
					break;
				case "mnch":
					$url = MNCH_BASE_URL."get_custom_fields.php";
			...
			if($url != ""){
				$random = rand(0,99999999).date("Y").date("m");
				$validationcode = md5('mns'.date("Ymd").'to'.$random.$mis);
