<?php 
/* 
*	Description		Server side code for visits confirmation of M&S version 10.0
*	Changes			---------------------------
*					1. Device date/time.
...
class VcApp_v3 extends CI_Controller {
	...
	*/
	...
	{
		echo "Moon: rajaimranqamer@gmail.com";
	}
	//-------------------------------------------------------
	/* 
	...
	*/
	public function login()
	{
		...
		$responce=array();
		if($IMEI_NO && $username && !$password){
			$user=$this -> VcApp_model -> login(array('username' => $username, 'imei_no' => $IMEI_NO));
			if ($user){
		...
		}else if ($IMEI_NO && $username && $password){
	...
	*/
	public function upcomming_visits()
	{
		...
		$where=array();
		if($IMEI_NO){
			$user=$this -> Common_model -> get_info("users",$IMEI_NO,"imei_no");
			if ($user){
						...
						for($i=0;$i<count($_POST["vpvids"]);$i++){
							...
							if($vpv){
								$dataToSave = array(
									...
									"device_date_confirmed"=> ($_POST['device_date'])?$_POST['device_date']:NULL,
									"device_time_confirmed"=> ($_POST['device_time'])?$_POST['device_time']:NULL,
									"device_ip"=> ($_POST['device_ip'])?$_POST['device_ip']:NULL,
									"imei_no"=> $IMEI_NO
								);
